...
describe("OperationList.diff", () => {
    ...
    it("should detect deleted entries", () => {
        ...
        const entries = Array.from(history1.entries()).filter(e => e.id !== 2);
    ...
    it("should detect changed entries (different instances)", () => {
        const history1 = OperationList.empty<number, string>(generateId).add("op1");
        const entry1 = history1.entry(1)!;

        // Entry with the same ID but different operation
        const entry2: OperationList.Entry<number, string> = { ...entry1, operation: "op1-modified" };
